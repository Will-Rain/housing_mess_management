...
layui.define(["layer"], function (exprots) {
    ...
    var okUtils = {
            ...
            $.ajax({
                ...
                success: function (data,status) {
                    if (status === "success") {
                        ...
                        // 业务异常
                        layer.msg(data.toString(), {icon: 7, time: 2000});
                        deferred.reject("okUtils.ajax warn: " + data.toString());
                    }
                },
                complete: function () {
                    if (load) {
         ...
         * @param key
         */
        tableBatchCheck: function (table,key) {  //传入主键，对主键进行审查
            ...
            if (rows > 0) {
                ...
                for (let i = 0; i < checkStatus.data.length; i++) {
                    ...
                    // Array[i] = checkStatus.data[i].name;
                    if(key === 'name')
                        Array.push(checkStatus.data[i].name);
                    else if(key === 'id')
                        Array.push(checkStatus.data[i].id);
                }
                return Array;
            } else {
